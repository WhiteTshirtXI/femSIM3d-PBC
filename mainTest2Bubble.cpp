...
//-------------------------------------------------- 
 ...
 const char *meshNew = "../../db/mesh/3d/test2bubble.vtk";
 Model3D m2;
 m2.readVTK(meshNew);
 m2.setMiniElement();
 //m2.setOFace();
 Simulator3D s5(m2,s1);
 s5.setRe(100);
 s5.setSc(2);
 s5.setWe(2);
 s5.setAlpha(1);
 s5.setBeta(-2.0);
 s5.setCflBubble(5/2);
 s5.setSolverPressure(new PetscSolver(KSPGMRES,PCILU));
 s5.setSolverVelocity(new PetscSolver(KSPCG,PCICC));
 s5.setSolverConcentration(new PetscSolver(KSPCG,PCICC));
 InOut save1(m2,s5); // cria objeto de gravacao
 save1.saveVTK(dir,vtk,9999);
 InOut save2(m1,s1); // cria objeto de gravacao
 save2.saveVTK(dir,vtk,2222);
